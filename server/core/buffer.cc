...
void GWBUF::set_custom_data(void* new_data, void (* deleter)(void*))
{
    auto& obj = sbuf->custom_data;
    mxb_assert(obj.data == nullptr && obj.deleter == nullptr);
    mxb_assert(!new_data || deleter);   // If data is given, a deleter must also be set.
    obj.data = new_data;
    obj.deleter = deleter;
    sbuf->info |= GWBUF_INFO_PARSED;    // Set flag
}

void* GWBUF::get_custom_data() const
{
    return sbuf->custom_data.data;
}
