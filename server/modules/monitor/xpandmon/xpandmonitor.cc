...
void XpandMonitor::post_loop()
{
    ...
    // NOTE: the right thing to do would be here deactivate all volatile servers.
    // NOTE: However, that would mean that the number of deactivated volatile
    // NOTE: servers hanging around could quickly grow if the monitor is directly
    // NOTE: or indirectly (e.g. alter monitor) stopped and started.
