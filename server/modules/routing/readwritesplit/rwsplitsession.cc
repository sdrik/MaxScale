...
bool RWSplitSession::can_start_trx_replay() const
{
    ...
    if (m_can_replay_trx)
    {
        ...
        {
            // m_trx_replay_start is only set when the first replay starts, this is why we must check how many
            // attempts we've made.
            if (m_num_trx_replays == 0 || trx_replay_time() < m_config.trx_timeout)
            {
                can_replay = true;
            }
            else
    ...
    if (m_config.transaction_replay && can_start_trx_replay())
    {
        ...
        if (m_trx.have_stmts() || m_current_query.get())
        {
            ...
            MXS_INFO("Starting transaction replay %ld. Replay has been ongoing for %ld seconds.",
                     m_num_trx_replays, trx_replay_time().count());
            m_state = TRX_REPLAY;
